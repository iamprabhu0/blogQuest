...
def parse_args():
    ...
    parser.add_argument("--feed-path", default="feeds.txt")
    parser.add_argument("--db-path", default="feeds.db")
    return parser.parse_args()


async def main(feed_file, db_path):
    ...
    c = conn.cursor()
    c.execute('''CREATE VIRTUAL TABLE IF NOT EXISTS feeds USING fts5(url, content)''')
    c.executemany("INSERT INTO feeds (url, content) VALUES (?, ?)", flattened_results)
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    args = parse_args()
    asyncio.run(main(args.feed_path, args.db_path))
